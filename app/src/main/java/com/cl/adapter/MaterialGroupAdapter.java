...
public class MaterialGroupAdapter extends RecyclerView.Adapter<MaterialGroupAdapter.ViewHolder> {

    private List<MaterialGroupListEntity> mList = new ArrayList<>();
    private Context mContext;
    public static final int ITEM_TYPE = 1, FOCUS_TYPE = 2;

    public MaterialGroupAdapter(Context pContext) {
        mContext = pContext;
    }

    public void setList(List<MaterialGroupListEntity> mList) {
        this.mList.addAll(mList);
        notifyDataSetChanged();
    }

    public List<MaterialGroupListEntity> getList() {
        return this.mList;
    }

    public void setReList(List<MaterialGroupListEntity> mList) {
        this.mList.clear();
        this.mList.addAll(mList);
        notifyDataSetChanged();
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        MaterialGroupListEntity entity = mList.get(position);
